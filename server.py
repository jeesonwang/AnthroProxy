...
# These are all populated by reload_config() below
...
def _load_config_from_env() -> dict:
    """Read initial config from environment variables (used to seed config.json)."""
    return {
...
def load_config() -> dict:
    """Load config from config.json, creating it from .env if it doesn't exist."""
...
def reload_config():
    """Re-read config.json and update all global variables."""
    global ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY
    global VERTEX_PROJECT, VERTEX_LOCATION, USE_VERTEX_AUTH
    global OPENAI_BASE_URL, PREFERRED_PROVIDER, BIG_MODEL, SMALL_MODEL
    global EXTRA_PARAMS
    ...
    VERTEX_PROJECT = cfg.get("vertex_project", "unset")
    VERTEX_LOCATION = cfg.get("vertex_location", "unset")
    USE_VERTEX_AUTH = cfg.get("use_vertex_auth", False)
    OPENAI_BASE_URL = cfg.get("openai_base_url", "") or None
    PREFERRED_PROVIDER = cfg.get("preferred_provider", "openai").lower()
    BIG_MODEL = cfg.get("big_model", "gpt-4.1")
    SMALL_MODEL = cfg.get("small_model", "gpt-4.1-mini")
    EXTRA_PARAMS = cfg.get("extra_params", {})
    logger.info(
        f"Config reloaded: provider={PREFERRED_PROVIDER}, big={BIG_MODEL}, small={SMALL_MODEL}"
    )


# Initialize from config.json (or .env on first run)
reload_config()


# Helper function to get model provider dynamically
def get_model_provider(model_name: str) -> str:
    """Uses LiteLLM to detect the provider for a given model name."""
    ...
    try:
        ...
        # Standardize gemini to google for consistency in our logic
        return provider
    except Exception:
        return "unknown"
...
class MessagesRequest(BaseModel):
    ...
    def validate_model_field(cls, v, info):  # Renamed to avoid conflict
        ...
        # Map Haiku to SMALL_MODEL based on provider preference
            ...
            small_model_provider = get_model_provider(SMALL_MODEL)
            if PREFERRED_PROVIDER == "google" and small_model_provider == "google":
        ...
        # Map Sonnet to BIG_MODEL based on provider preference
            ...
            big_model_provider = get_model_provider(BIG_MODEL)
            if PREFERRED_PROVIDER == "google" and big_model_provider == "google":
        ...
        # Add prefixes to non-mapped models if they match known lists
            ...
            detected_provider = get_model_provider(clean_v)
            if detected_provider == "google" and not v.startswith("gemini/"):
                new_model = f"gemini/{clean_v}"
                mapped = True
            elif detected_provider == "openai" and not v.startswith("openai/"):
...
async def log_requests(request: Request, call_next):
    ...
    logger.debug(
        f"Request: {method} {path}, Time Cost: {time.time() - start_time:.2f}s, Body: {request_body.decode(errors='ignore')[:100]}..."
    )
...
def _extract_tool_result_text(b: "ContentBlockToolResult") -> str:
    ...
    if isinstance(b.content, dict):
        if b.content.get("type") == "text":
            return str(b.content.get("text", ""))
        try:
            return json.dumps(b.content)
        except Exception:
...
def convert_anthropic_to_litellm(anthropic_request: MessagesRequest) -> Dict[str, Any]:
    ...
    if anthropic_request.system:
        ...
        if isinstance(anthropic_request.system, str):
            ...
            system_text = ""
            for sys_block in anthropic_request.system:
                if isinstance(sys_block, SystemContent):
                    system_text += sys_block.text + "\n\n"
                elif isinstance(sys_block, dict) and sys_block.get("type") == "text":
                    system_text += str(sys_block.get("text", "")) + "\n\n"
    ...
    for msg in anthropic_request.messages:
        ...
        if isinstance(msg_content, str):
            ...
            for b in msg_content:
                if isinstance(b, ContentBlockToolResult):
                    has_tool_result = True
                    break

            is_openai_target = anthropic_request.model.startswith("openai/")

            if is_openai_target and has_tool_result:
                # OpenAI format: each tool_result becomes a separate {"role": "tool", ...} message.
                # Any leading text content is emitted first as a user message.
                text_parts: List[str] = []
                for b in msg_content:
                    if isinstance(b, ContentBlockText):
                        ...
                        if text_parts:
                            ...
                            text_parts = []
                        result_content = _extract_tool_result_text(b)
                        messages.append(
                            {
                                "role": "tool",
            ...
            elif is_openai_target and msg.role == "assistant":
                # OpenAI format: assistant tool_use blocks → tool_calls field.
                tool_calls_list: List[Dict[str, Any]] = []
                text_parts_asst: List[str] = []
                for b in msg_content:
                    if isinstance(b, ContentBlockText):
                        text_parts_asst.append(b.text)
                        ...
                        tool_calls_list.append(
                            {
                                ...
                                "function": {
                                    ...
                                    if not isinstance(b.input, str)
                                    else b.input,
                                },
                            }
                        )
                asst_msg: Dict[str, Any] = {"role": "assistant"}
                combined_text = "\n".join(text_parts_asst).strip()
                asst_msg["content"] = combined_text if combined_text else None
                if tool_calls_list:
                    asst_msg["tool_calls"] = tool_calls_list
                messages.append(asst_msg)

            else:
                # Default handling: Anthropic-style content blocks (used for anthropic/ models
                # or non-tool user messages for openai/ models).
                processed_content: List[Dict[str, Any]] = []
                for b in msg_content:
                    if isinstance(b, ContentBlockText):
                        processed_content.append({"type": "text", "text": b.text})
                    elif isinstance(b, ContentBlockImage):
                        processed_content.append({"type": "image", "source": b.source})
                    elif isinstance(b, ContentBlockToolUse):
                        ...
                        if b.content:
                            if isinstance(b.content, str):
                                ...
                                processed_content_block["content"] = b.content
                            else:
                                processed_content_block["content"] = [
                                    {"type": "text", "text": str(b.content)}
                                ]
                        else:
                            processed_content_block["content"] = [
                                {"type": "text", "text": ""}
    ...
    # Apply global EXTRA_PARAMS overrides
    if EXTRA_PARAMS.get("temperature") is not None:
        litellm_request["temperature"] = EXTRA_PARAMS["temperature"]
    if EXTRA_PARAMS.get("top_p") is not None:
        litellm_request["top_p"] = EXTRA_PARAMS["top_p"]
    # Build extra_body for chat_template_kwargs / enable_thinking.
    # When tools are present, disable thinking mode: thinking models express tool calls
    # as plain text ([Tool: ...]) rather than structured API tool_calls when thinking is
    # enabled, causing the proxy to receive finish_reason="stop" instead of "tool_calls".
    has_tools = bool(anthropic_request.tools)
    extra_body: Dict[str, Any] = {}
    if EXTRA_PARAMS.get("enable_thinking") is not None:
        thinking_value = False if has_tools else EXTRA_PARAMS["enable_thinking"]
        extra_body.setdefault("chat_template_kwargs", {})["enable_thinking"] = (
            thinking_value
        )
    if EXTRA_PARAMS.get("chat_template_kwargs"):
        for k, v in EXTRA_PARAMS["chat_template_kwargs"].items():
            if k == "enable_thinking" and has_tools:
                extra_body.setdefault("chat_template_kwargs", {})[k] = False
            else:
                extra_body.setdefault("chat_template_kwargs", {})[k] = v
    if extra_body:
        litellm_request["extra_body"] = extra_body
...
def convert_litellm_to_anthropic(
    ...
    try:
        ...
        # Convert tool_calls to Anthropic tool_use content blocks (works for all model types)
        if tool_calls:
            ...
            for idx, tool_call in enumerate(tool_calls):
                ...
                logger.debug(
                    f"Adding tool_use block: id={tool_id}, name={name}, input={args_dict}"
                )
        ...
        if isinstance(usage_info, dict):
            ...
            prompt_tokens = int(getattr(usage_info, "prompt_tokens", 0))
            completion_tokens = int(getattr(usage_info, "completion_tokens", 0))
        ...
        if finish_reason == "stop":
            ...
            stop_reason_str = "end_turn"  # Default
...
async def handle_streaming(
    response_generator: AsyncIterator[Any], original_request: MessagesRequest
):
...
async def create_message(request: MessagesRequest, raw_request: Request):
    try:
        ...
        if request.model.startswith(("openai/", "vllm/")):
        ...
        if request.stream:
            ...
            litellm_request["max_completion_tokens"] = 32768
            litellm_request["max_tokens"] = 121842
        ...
        status_code_val = error_details.get("status_code", 500)
        try:
            status_code = int(status_code_val)
...
async def get_config():
    """Return current config with API keys masked for display."""
    cfg = load_config()
    ...
    return {
        **cfg,
        ...
        "anthropic_api_key": mask(cfg.get("anthropic_api_key", "")),
    }


@app.post("/api/config")
async def save_config(payload: ConfigPayload):
    """Save config to config.json and hot-reload global variables."""
    current = load_config()

    def resolve_key(new_val: str, current_val: str) -> str:
        """If new_val looks like a masked key (contains ****), keep current."""
        if "****" in new_val:
            return current_val
        return new_val
    ...
    cfg = {
        ...
        "big_model": payload.big_model,
        "small_model": payload.small_model,
        "openai_api_key": resolve_key(
            payload.openai_api_key, current.get("openai_api_key", "")
        ),
        "openai_base_url": payload.openai_base_url,
        "gemini_api_key": resolve_key(
            payload.gemini_api_key, current.get("gemini_api_key", "")
        ),
        "anthropic_api_key": resolve_key(
            payload.anthropic_api_key, current.get("anthropic_api_key", "")
        ),
        "use_vertex_auth": payload.use_vertex_auth,
        "vertex_project": payload.vertex_project,
        "vertex_location": payload.vertex_location,
        "extra_params": payload.extra_params.model_dump(),
    }
    CONFIG_PATH.write_text(json.dumps(cfg, indent=2))
    reload_config()
